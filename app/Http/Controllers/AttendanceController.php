...
class AttendanceController extends AccountBaseController
{
    ...
    public function bulkMark(StoreBulkAttendance $request)
    {
        ...
        if ($request->mark_attendance_by == 'month') {
            ...
            $startDate = Carbon::createFromFormat($this->company->date_format, $dates[0]);
            $endDate = Carbon::createFromFormat($this->company->date_format, $dates[1]);
        }
    ...
    public function qrClockInOut(Request $request)
    {
        ...
        $company = Company::select('id', 'timezone')->where('id', $user->company_id)->first();
