...
class CurrencySettingController extends AccountBaseController
{
    ...
    public function exchangeRate($currency)
    {
        ...
        try {
            // Get exchange rate
            $client = new Client();
            $res = $client->request('GET', 'https://' . $currencyApiKeyVersion . '.currconv.com/api/v7/convert?q=' . companyOrGlobalSetting()->currency->currency_code . '_' . $currency . '&compact=ultra&apiKey=' . $currencyApiKey);
            $conversionRate = $res->getBody();
            $conversionRate = json_decode($conversionRate, true);
            $rate = $conversionRate[mb_strtoupper(companyOrGlobalSetting()->currency->currency_code) . '_' . $currency];
